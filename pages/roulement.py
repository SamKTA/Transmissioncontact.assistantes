...
def show():
    ...
    def create_rotation_card(column, rotation_type, color):
        with column:
            ...
            next_counselor = get_next_counselor(rotation_type, last_counselor, indispo_df)
            ...
            st.markdown(
                ...
                <div class="rotation-card" style="background-color: white; border-radius: 10px; padding: 1rem; 
                    ...
                    <p><strong>Conseiller suivant recommandé:</strong> {next_counselor or "Non déterminé"}</p>
                </div>
                """,
                unsafe_allow_html=True
            )
            ...
            with col_select:
                if st.button(f"Sélectionner", key=f"btn_select_{rotation_type}"):
                    ...
                    st.session_state.conseiller_selectionne = CONSEILLERS.get(next_counselor, next_counselor)
                    change_page("formulaire")
            
            with col_skip:
                if st.button(f"Suivant", key=f"btn_skip_{rotation_type}"):
                    # Gérer le saut de conseiller
                    skipped_counselor = next_counselor
                    next_after_skip = handle_skip(rotation_type, last_counselor, indispo_df)
                    
                    # Notification et mise à jour
                    st.warning(f"Conseiller {skipped_counselor} sauté. Nouveau conseiller proposé: {next_after_skip}")
